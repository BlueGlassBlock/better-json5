			...
			for (const association of associations) {
				if (association.fullUri === uri) {
					return {
						...
						buttonCommands: [() => commands.executeCommand(association.workspaceFolder ? 'workbench.action.openWorkspaceSettingsFile' : 'workbench.action.openSettingsJson', ['json5.schemas'])]
					};
				}
			}
			return undefined;
...
function showSchemaList(input: ShowSchemasInput) {
	...
	if (items.length === 0) {
		items.push({
			...
			buttons: [{ iconPath: new ThemeIcon('gear'), tooltip: l10n.t('Open Settings') }],
			buttonCommands: [() => commands.executeCommand('workbench.action.openSettingsJson', ['json5.schemas'])]
		});
	}
...
export function createDocumentSymbolsLimitItem(documentSelector: DocumentSelector, settingId: string, limit: number): Disposable {
	const statusItem = languages.createLanguageStatusItem('json5.documentSymbolsStatus', documentSelector);
